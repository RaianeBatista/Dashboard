...
export function TabItem({ value, title, isSelected = false }: TbaItemProps) {
  return (
    <Tabs.Trigger
      value={value}
      className='relative group px-1 pb-4 text-zinc-500 hover:text-violet-700 data-[state=active]:text-violet-700 outline-none'
    >
      {/* <span className='group-focus-visible:ring-2 group-focus-visible:ring-violet-400 rounded group-focus-visible:ring-offset-4'
      >{title}</span> */}
      <span className='group-focus:ring-2 group-focus:ring-violet-400 rounded group-focus:ring-offset-4'
      >{title}</span>
